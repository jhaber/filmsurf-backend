...
var retrieveByImdbPath = host + basePath + '/movie_alias.json';
...
exports.retrieveByImdb = function(imdbId) {
    var deferred = Q.defer();

    var request = {};
    request.url = retrieveByImdbPath;
    request.qs = { 'id': imdbId.substring(2),
        'type': 'imdb',
        'apikey': apiKey };
    request.json = true;
    ...
    Request.get(request, function (err, response, body) {
        if (!err && response && response.statusCode === 200) {
            ...
            if (err) {
                console.log('Rotten Tomatoes error: ' + JSON.stringify(err));
                deferred.reject(err);
            } else if (response) {
                console.log('Rotten Tomatoes error: ' + JSON.stringify(response));
                deferred.reject(response);
            } else {
                console.log('Rotten Tomatoes error');
                deferred.reject();
            }
        }
    });
