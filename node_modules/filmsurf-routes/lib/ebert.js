...
exports.test = function () {
    ...
    var lastRun = DB.last_run ? new Date(DB.last_run) : undefined;
    DB.last_run = new Date();

    keepScraping(DB, lastRun, 1).then(function(DB) {
        fs.writeFileSync(__dirname + '/../lib/ebert-db.json', JSON.stringify(DB), 'utf8');
    });
};

function keepScraping(DB, lastRun, page) {
    return scrapePage(DB, lastRun, page).then(function(shouldContinue) {
        if (shouldContinue) {
            return keepScraping(DB, lastRun, page + 1);
        } else {
            return Q.resolve(DB);
        }
    });
...
function scrapePage(DB, lastRun, page) {
    return retrievePage(page).then(function(htmlString) {
        var reviews = parseHtml(htmlString);

        return transformReviews(reviews, lastRun).then(function(results) {
            var newReviews = results.reviews;
            var newErrored = results.errored;
            ...
            for (var j = 0; j < newErrored.length; j++ ) {
                var errored = newErrored[i];
                DB.errored.push(errored);
            }
...
function retrievePage(page) {
    ...
    var url = 'http://www.rottentomatoes.com/critic/roger-ebert/?page=' + page;
...
function transformReviews(reviews, maxAge) {
    var promises = [];

    var nextPage = false;
    for (var i = 0; i < reviews.length; i++) {
        var review = reviews[i];
        if (!maxAge || !review.date.isBefore(maxAge)) {
    ...
    return Q.allResolved(promises).then(function(promises) {
        var successfulReviews = [];
        var failedReviews = [];

        promises.forEach(function(promise) {
            if (promise.isFulfilled()) {
                successfulReviews.push(promise.valueOf());
            } else {
                failedReviews.push(promise.valueOf().exception);
            }
        });

        return {
            'reviews': successfulReviews,
            'errored': failedReviews,
            'continue': nextPage
        }
    });
}
...
function fetchRottenTomatoesId(review) {
    ...
    if (rtUrlCache[review.rt_url]) {
        deferred.resolve(rtUrlCache[review.rt_url]);
        console.log('Cache hit: ' + review.rt_url);
    } else {
        rtLimiter.removeTokens(1, function() {
            Request.get(review.rt_url, function(err, response, body) {
                if (!err && response && response.statusCode === 200 && body) {
                    var rtId = findRottenTomatoesId(body);
                    rtUrlCache[review.rt_url] = rtId;
